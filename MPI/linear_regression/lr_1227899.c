...
int main() {
  ...
  while(!minimum_found) {
		if(rank == 0){
    for(i=0;i<8;i++){
			MPI_Send(&dmdc, 2, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD);
		}
		for(i=0;i<8;i++){
			MPI_Recv(&e[i], 1, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD, &status);
		}
      dm[i] = bm + (om[i] * step);
      dc[i] = bc + (oc[i] * step);    
    }
   ...
   // printf("best m,c is %lf,%lf with error %lf in direction %d\n", 
    ...
    if(best_error < be) {
      ...
      minimum_found = 1;
		...
		for(i=0;i<8;i++){
			MPI_Send(&minimum_found, 1, MPI_INT, 1, 0, MPI_COMM_WORLD);
		}
  }else{
		MPI_Recv(&e[i], 1, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD, &status);
		er = rms_error(dmdc.dm, dmdc.dm);
